    ...
    <div style="overflow: auto;">
        <table>
            ...
            <?PHP
                ...
                {
                    $transfers = json_decode(file_get_contents("https://fantasy.premierleague.com/api/entry/".$tasS['entry']."/event/".$leagues['current_event']."/picks/"), true);
            ?>
                <tr style="text-align:center;">
                    <td>
                        <?PHP echo $tasS['rank'];
                        if($tasS['rank'] < $tasS['last_rank']) {
                        ...
                        } elseif($tasS['rank'] > $tasS['last_rank'] && $tasS['last_rank'] != 0) {
                    ...
                    <td class="nameCol" style="text-transform:capitalize;">
                        <?PHP
                        echo '<span class="name" style="font-weight:bold;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:inline-block;vertical-align:bottom;">'.$tasS['player_name'].'</span>';
                    ...
                    <td><?PHP echo $tasS['total']; ?></td>
                    <td>
                        <?PHP 
                        $difference = $tasStandings['standings']['results'][0]['total'] - $tasS['total'];
                        if($difference === 0) {
                            echo "-";
                        } else {
                            echo $difference;
